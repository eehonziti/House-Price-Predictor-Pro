...
with st.sidebar:
    ...
    n_estimators = st.slider("RF: n_estimators", 50, 300, 150, 50)
    max_depth = st.slider("RF: max_depth", 4, 20, 12, 2)
...
def load_everything(test_sz, seed, n_est, mx_depth):
    ...
    models_def = {
        ...
        "Extra Trees":          ExtraTreesRegressor(n_estimators=n_est, max_depth=mx_depth,
                                                    random_state=seed, n_jobs=-1),
...
with tabs[5]:
    ...
    with st.spinner("Running cross-validation…"):
        for name, mdl in trained_models.items():
            Xc = X_train_sc if name in ("Ridge Regression","Lasso Regression") else X_train
            scores = cross_val_score(mdl, Xc, y_train, cv=5, scoring="r2", n_jobs=-1)
