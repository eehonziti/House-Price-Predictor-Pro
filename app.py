...
.metric-card .label { font-size: 0.75rem; color: #7878A0; text-transform: uppercase; letter-spacing: 1.2px; }
.metric-card .value { font-size: 1.85rem; font-weight: 700; color: #E8E8F0; line-height: 1.2; }
...
.scenario-card h4 { margin: 0 0 0.3rem; font-size: 0.9rem; color: #9898C0; }
.scenario-card .sc-price { font-size: 1.4rem; font-weight: 700; color: #64B5F6; }
...
.stTabs [data-baseweb="tab-list"] { background: #1A1A2E !important; border-radius: 10px; }
.stTabs [data-baseweb="tab"][aria-selected="true"] { background: #0F3460 !important; border-radius: 8px; }
...
[data-testid="stToolbar"] { background: #0D0D1A !important; }
...
/* The inner glide-data-grid canvas renders itself, but the wrapper needs dark bg */
...
/* ── Selectbox / Dropdown — full dark override ── */
/* Trigger button */
...
/* The value text inside the select box */
[data-baseweb="select"] span,
[data-baseweb="select"] div[class*="placeholder"],
[data-testid="stSelectbox"] span { color: #E8E8F0 !important; }
/* Dropdown chevron/icon */
[data-baseweb="select"] svg { fill: #9898C0 !important; }

/* ── Native Streamlit widgets dark polish ── */
hr { border-color: #2A2A40 !important; }
p, li, span, label { color: #C8C8E0; }
h1, h2, h3, h4 { color: #E8E8F0; }
</style>
""", unsafe_allow_html=True)
...
# ── Fixed training config (never changes, so cache never invalidates) ──
_TEST_SIZE    = 0.20
_SEED         = 42
_N_ESTIMATORS = 50   # kept low for fast cold-start (~15s on Streamlit Cloud)
_MAX_DEPTH    = 8

@st.cache_resource(show_spinner="🏗️ Training models (first load only — ~15 seconds)…")
def load_everything():
    """Train once, cache forever. No args = cache never invalidates on interaction."""
    from sklearn.datasets import fetch_california_housing
    ...
    models_def = {
        "Random Forest":     RandomForestRegressor(n_estimators=_N_ESTIMATORS, max_depth=_MAX_DEPTH,
                                                   random_state=_SEED, n_jobs=-1),
        "Extra Trees":       ExtraTreesRegressor(n_estimators=_N_ESTIMATORS, max_depth=_MAX_DEPTH,
                                                 random_state=_SEED, n_jobs=-1),
        "Gradient Boosting": GradientBoostingRegressor(n_estimators=_N_ESTIMATORS, max_depth=5,
                                                       random_state=_SEED, subsample=0.8),
        "Decision Tree":     DecisionTreeRegressor(max_depth=_MAX_DEPTH, random_state=_SEED),
